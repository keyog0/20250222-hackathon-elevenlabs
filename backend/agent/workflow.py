...
def create_agent_workflow(llm: LLMUtils, initial_state: AgentState) -> StateGraph:
    ...
    def should_continue(state: AgentState) -> Union[str, END]:
        ...
        if (
            state.get("response", {}).get("metadata", {}).get("scenario_changed", False) is False
            and state.get("response", {}).get("metadata", {}).get("completion_reason") is not None
        ):
            return END
    ...
    workflow.add_conditional_edges("process_sequential", should_continue, {END: END})
...
async def run_agent(workflow: StateGraph, user_input: str, state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"response": final_state["response"]["response_text"], "metadata": final_state["response"]["metadata"]}
    except Exception as e:
